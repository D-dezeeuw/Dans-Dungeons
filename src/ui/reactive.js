...
export function registerReactiveSidebar() {
  ...
  // `ui.pcStats` / `ui.enemyStats` reach the DOM via `:innerHTML` bindings in
  ...
  // TTS state — drive aria-pressed on the toggle button.
  computed('ui.ttsActive',     ['settings.tts'],        s => !!(s.settings?.tts));

  // TTS toggle icon — the SVG is bound into the button via `:innerHTML` in
  // index.html. Both strings are trusted static markup (no interpolation).
  ...
  computed('ui.ttsIconHtml',   ['settings.tts'],        s => s.settings?.tts ? _volOn : _volOff);
